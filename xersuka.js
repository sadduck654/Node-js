//🎄Андрей, добрый день!
//✅А добрый потому что мы подготовили рандомный промокод для вас😋
//Промокод 👉 Ди2941
//Чтобы активировать промокод - напишите боту: Промокод Ди2941🤖
...
let user = new VK();
user.setOptions({
	token: '687af957cc40204e3c5daf7e0f80e6ada4642974cb303b32b3f6fcc25ed0b04021543428c635f98008390' // token ot страницы ( не обязательно )
});
let giving = false;
const cars = [
	{
...
vk.setOptions({ token: '687af957cc40204e3c5daf7e0f80e6ada4642974cb303b32b3f6fcc25ed0b04021543428c635f98008390', pollingGroupId:201428905 });
...
updates.on('message', async (message) => {
	...
	{
		message.send (`[👋🏻]Привет,я игровой бот "Бот Kair".
...
🔹Что бы купить донат, перейдите по ссылке vk.com/andrey_erenov
`)
});
...
🔹Что бы купить Валюту, перейдите по ссылке vk.com/andrey_erenov
`)
});
/////
...
owner_id: 590200489, ///owner_id свой или айди того кому хотите накрутить 
post_id: 4, ///post_id айди поста
from_group: 201428905, /// ну а тут айди группы от имени которой вы хотите накрутить 
message: `Ура!` 
}); 
}, 500); 
...
🔸 ➾ 7. Выдача себя за Sozdatel/admin/moder. 
...
let prize = utils.pick([1]);
	...
	{
		message.user.pass = 1;
		return bot(`🧔 Здравствуйте , я Еренов Андрей Павлович !
...
if(message.user.pass < 1) return bot('У вас нету Паспорта!');
...
let prize = utils.pick([1]);
    if (prize === 1) 
    {
        ...
        let iin = Math.random() * 999999999999; 
        iin = Math.round(iin);
        
        return bot(`Паспорт гражданина республики ${stranas[message.user.transport.strana - 1].name}\nИмя: ${message.user.tag}\nНациональность: Русский\nID: ${message.user.uid}\nМесто работы: ${works[message.user.work - 1].name}\nСудимость: ${message.user.warn}\nПричина: ${message.user.warn_p}\nДата регистрации: ${message.user.regDate}`);
    }
});
